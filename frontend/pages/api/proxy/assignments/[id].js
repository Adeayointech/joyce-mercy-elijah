// Simple proxy route so browser can download files from backend with Authorization preserved
import fetch from 'node-fetch'

export default async function handler(req, res){
  const { id } = req.query
  const apiUrl = `${process.env.NEXT_PUBLIC_API_BASE || 'http://localhost:4000'}/assignments/${id}/download`
  ...
  try{
    ...
    // forward content-type and disposition
